...
func (s funcData) npcdata() int   { return int(s.field(7)) }
func (s funcData) nfuncdata() int { return int(s.field(10) & 255) }
...
func (f *Func) TableSizePCData(tab int) int {
	...
	if f.LineTable.version >= ver116 {
		tableOff = fs.field(uint32(10 + tab))
	} else {
		tableOff = fs.field(uint32(8 + tab))
	}
	if tableOff == 0 {
		return 0
	}
